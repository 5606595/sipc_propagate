...
var distance = 0;
var drawId;
var flameHeight = 0, centerXOffset = 40, flag = 1;
...
function draw(){
    ...
    if(distance >= 10000) {
        ...
        shipCenter.y -= 10;
        flameHeight = speed / 3;
        if(shipCenter.y < -300 && flag) {
            // drawFlames = function(p, color){
            //     centerXOffset += 3;
            //     flameHeight += 500 * (speed / 3);
            //     ship.beginPath();
            //     ship.moveTo(shipCenter.x, shipCenter.y + 110);
            //     ship.quadraticCurveTo(shipCenter.x + centerXOffset, shipCenter.y + 110, flameEndX, flameHeight);
            //     ship.quadraticCurveTo(shipCenter.x - centerXOffset, shipCenter.y + 110, shipCenter.x, shipCenter.y + 110);
            //     ship.fillStyle = '#FFF';
            //     ship.fill();
            //     ship.closePath();
            // }
            $(".distance").css('opacity', '0');
            $("#hj h1").css('opacity', '0');
            window.setTimeout(function() {
                ...
                window.setTimeout(function() {
                    ...
                    $(".page1").addClass('ani aniDisplay');
                    // $("#container").css('opacity', 'block');
                    // $("#compress").css('opacity', '1')
                    $("#second .img1").addClass('imgAnimation');
                }, 200);
            }, 2000);
            flag = 0;
        }
...
$(".img1").on('webkitAnimationEnd', function() {
    $("#second .img1").css('opacity', '1');
    $("#second .img2").addClass('imgAnimation');
})

$(".img2").on('webkitAnimationEnd', function() {
    $("#second .img2").css('opacity', '1');
    $("#second .img3").addClass('imgAnimation');
})

$(".img3").on('webkitAnimationEnd', function() {
    $("#second .img3").css('opacity', '1');
})
...
function updateDistance(){
    if(distance >= 10000) {
        ...
        $('[data-sipc]').text(10000 - parseInt(distance));
        return ;
    }
    distance += speed;
    $('[data-distance]').text(parseInt(distance));
